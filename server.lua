...
if Config.Framework == "ESX" then
    Framework.RegisterServerCallback('SoUpdateModif', function(source, cb, price)
        ...
        IsVehicleOwnedByJob(plate, function(isJobVehicle, vehicleJob)
            if isJobVehicle and playerJob == vehicleJob and Config.EnableSocietyPay then
                ...
                if HasEnoughSocietyMoney(vehicleJob, finalPrice) then
                    ...
                    NotifyPlayer(source, '~g~Société a payé: ' .. finalPrice..'$')
                    cb(true)
                else
                    NotifyPlayer(source, '~r~La société n\'a pas assez de fonds')
                    cb(false)
                end
            else
                -- Paiement personnel
                if HasEnoughMoney(source, finalPrice) then
                    ...
                    NotifyPlayer(source, '~g~Vous avez payé: ' .. finalPrice..'$')
                    cb(true)
                else
                    NotifyPlayer(source, '~r~Vous n\'avez pas assez d\'argent')
                    cb(false)
                end
            end
        end)
...
elseif Config.Framework == "QBCore" then
    Framework.Functions.CreateCallback('SoUpdateModif', function(source, cb, price)
        local playerJob = GetPlayerJob(source)
        local finalPrice = Config.PaymentFunctions.ApplyDiscount(source, price, playerJob)
...
AddEventHandler('SoUpdate', function(plate, props)
    ...
    TriggerClientEvent('SoSyncV', -1, NetworkGetNetworkIdFromEntity(GetVehiclePedIsIn(GetPlayerPed(source), false)), props)
end)

-- Événement pour synchroniser les modifications entre tous les joueurs
RegisterServerEvent('SoSyncV')
AddEventHandler('SoSyncV', function(netId, props)
    TriggerClientEvent('SoSyncV', -1, netId, props)
end)